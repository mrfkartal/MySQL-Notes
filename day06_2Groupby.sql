...
select ulke (avg(maas))as ulkelere_gore_maas_ort from personel group by ulke;
select ulke, round(avg(maas))as ort_maas from personel group by ulke;


-- SORU03: Her ülkede kaç çalışan olduğunu sorgulayınız.
select ulke, count(ad) as toplam_kisi_sayisi from personel group by ulke;
select ulke, count(ad)as kisi_sayisi from personel group by ulke;
...
select cinsiyet, round(avg(maas)) as ortlama_maas from personel group by cinsiyet;
...
select ulke, cinsiyet, count(*) as calisan_sayisi,
 round(avg(yas)) yas_ortalamasi from personel
group by ulke, cinsiyet;


-- SORU08: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  ve maası 
-- 30000 den büyük olanları sorgulayınız.
select ulke, cinsiyet, count(*) as kisi, round(avg(maas)) as yas_ortalamasi from personel 
where maas>30000 
group by ulke, cinsiyet;
