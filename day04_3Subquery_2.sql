...
INSERT INTO calisanlar VALUES(456789012, 'Veli Yilmaz', 'Ankara', 1000, 'Pierre Cardin');
...
-- as ile toplam maasa sutununa isim veriyoruz bu islem as olmadan da yapilir ve calisir
 
-- SORU5: Her markanin ismini, calisan sayisini ve o markaya ait calisanlarin ortalama maaşini listeleyen bir Sorgu yaziniz.

select marka_isim,calisan_sayisi,(select avg(maas) from calisanlar where isyeri=marka_isim) as ortalama_maas from markalar;
select marka_isim,calisan_sayisi,(select round(avg(maas)) from calisanlar where isyeri=marka_isim) as ortalama_maas from markalar; -- ortalamayi uyvarlar
select marka_isim,calisan_sayisi,(select round(avg(maas),2) from calisanlar where isyeri=marka_isim) as ortalama_maas from markalar; -- ortalamayi uyvarlar ve virgulden sonra 2 basamak kullanir
...
select marka_id, marka_isim, (select count(distinct sehir) from calisanlar where isyeri=marka_isim) as top_sehir from markalar;